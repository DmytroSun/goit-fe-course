...
const findLongestWord = function (string) {
  ...
  let wordLength = 0;
  let longestWord;
  for (let arrayItem of subArray) {
    if (arrayItem.length > wordLength) {
      wordLength = arrayItem.length;
      longestWord = arrayItem;
    }
  }
  return longestWord;
};
...
const formatString = function (someString) {
  ...
  if (someString.length < 40) {
    ...
    console.log("----------");
    console.log(
      `( FYI the original sentence lenght = ${someString.length} symbols.)`
    );
  } else {
    functionVar = someString.slice(0, 40) + "...";
    console.log("----------");
    console.log(
      `( FYI the original sentence lenght = ${
        someString.length
      } symbols. The modified length = ${functionVar.length -
        3} symbols + "..." )`
    );
  }
  return functionVar;
